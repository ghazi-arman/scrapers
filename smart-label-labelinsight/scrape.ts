...
function extractAllergenStatement($: cheerio.CheerioAPI): string | null {
  ...
  const modernContainer = $("[class*='AllergenList__Container']").first();
  if (modernContainer.length) {
    const contains = new Set<string>();
    const mayContain = new Set<string>();
    const shared = new Set<string>();
    modernContainer
      .find("[class*='AllergenList__AllergenItem'], a[href*='/allergens/detail/']")
      .each((_, el) => {
        const row = $(el);
        const name = normalizeWhitespace(
          row.find("[class*='AllergenList__Name']").first().text() || row.text()
        );
        const presence = normalizeWhitespace(
          row.find("[class*='AllergenList__Presence']").first().text()
        );
        if (!name || !presence) return;
        const lowerName = name.toLowerCase();
        if (/may\s*contain/i.test(presence)) mayContain.add(lowerName);
        else if (/contains?/i.test(presence)) contains.add(lowerName);
        else if (/shared\s*facility/i.test(presence)) shared.add(lowerName);
      });

    const statement = buildStatement(contains, mayContain, shared);
  ...
  return buildStatement(contains, mayContain, shared);
}

function extractLabelInsightIngredients($: cheerio.CheerioAPI): string | null {
  const byId = $("#ingredients");
  if (byId.length) {
    const text = normalizeWhitespace(byId.text());
    if (text) return text;
  }
  const byClass = $("[class*='ingredient'], [class*='Ingredient']")
    .filter((_, el) => /ingredient/i.test($(el).text()))
    .first();
  if (byClass.length) {
    const text = normalizeWhitespace(byClass.text());
    if (text) return text;
  }
  const bodyText = normalizeWhitespace($("body").text());
  const match = bodyText.match(/ingredients?:\s*([^]+?)(contains|allergen|nutrition|serving|$)/i);
  if (match?.[1]) {
...
function buildLabelInsightIngredientText(items: unknown[]): string {
  const sorted = items
    .map((item) => item as Record<string, unknown>)
    .sort((a, b) => String(a.order ?? "").localeCompare(String(b.order ?? "")));

  const renderItem = (item: Record<string, unknown>): string | null => {
    const name = normalizeWhitespace(String(item.name ?? ""));
    if (!name) return null;
    const sub = Array.isArray(item.subIngredients) ? item.subIngredients : [];
    if (sub.length === 0) return name;
    const renderedSubs = sub.map((s) => renderItem(s as Record<string, unknown>)).filter(Boolean) as string[];
    if (!renderedSubs.length) return name;
    const joined = joinSubIngredients(renderedSubs, false);
    return `${name} (${joined})`;
  };

  const rendered = sorted.map((i) => renderItem(i)).filter(Boolean) as string[];
  return rendered.join(", ");
}

function extractLabelInsightIngredientsFromJson(payload: any): string | null {
  const ingredients = payload?.ingredientSection?.ingredients;
  if (!Array.isArray(ingredients) || ingredients.length === 0) return null;
  const text = buildLabelInsightIngredientText(ingredients);
...
function parseSmartLabelNutrition($: cheerio.CheerioAPI): { nutrition: ScraperNutritionData | null; servingSizeText: string | null } {
  ...
  section.find(".nfp__row").each((_, row) => {
    ...
    const valueText = normalizeWhitespace($(row).find(".nfp__values-right").first().text());
...
function parseLabelInsightNutrition($: cheerio.CheerioAPI): { nutrition: ScraperNutritionData | null; servingSizeText: string | null } {
  const containers = [
    $(".nutrition-facts").first(),
    $(".nutritionFacts").first(),
    $("[class*='nutrition']").first(),
  ].filter((c) => c && c.length);
  for (const container of containers) {
    const fromTables = extractNutritionFromTables($, container);
    if (fromTables) return { nutrition: fromTables, servingSizeText: fromTables.serving_size_text ?? null };
    const text = normalizeWhitespace(container.text());
    const fallback = extractNutritionFromText(text);
    if (fallback.nutrition) return fallback;
  }
  const fallback = extractNutritionFromText($("body").text());
  return { nutrition: fallback.nutrition, servingSizeText: fallback.servingSizeText };
}

function extractJsonObjectAfterKey(html: string, key: string): Record<string, unknown> | null {
  ...
  for (let i = braceStart; i < html.length; i++) {
    ...
    if (ch === "}") depth -= 1;
    if (depth === 0) {
      const candidate = html.slice(braceStart, i + 1);
      try {
...
function extractLabelInsightProductJson(html: string): Record<string, unknown> | null {
  return extractJsonObjectAfterKey(html, "product");
}

function parseLabelInsightNutritionJson(payload: any): { nutrition: ScraperNutritionData | null; servingSizeText: string | null } {
  const panel = payload?.nutritionSection?.nutrientPanels?.[0];
  if (!panel) return { nutrition: null, servingSizeText: null };
  const nutrition: ScraperNutritionData = {
    serving_size_value: 1,
    serving_size_unit_text: "serving",
    serving_size_text: panel.servingSize || null,
  };
  if (panel.servingSize) {
    const parsed = parseServingSize(panel.servingSize);
    nutrition.serving_size_value = parsed.value ?? 1;
    nutrition.serving_size_unit_text = parsed.unit ?? "serving";
  }
  const nutrients = panel.nutrients || [];
  for (const n of nutrients) {
    ...
    if (/calories/i.test(name)) {
      const parsed = parseNutrientAmountWithQualifier(valueText);
      if (parsed) nutrition.calories = parsed.value;
      continue;
    }
  ...
  return { nutrition, servingSizeText: nutrition.serving_size_text ?? null };
}
    ...
    while (true) {
      ...
      const hrefs = await page.$$eval("a[href]", (links) =>
        links
          ...
          .filter((href) =>
            /smartlabel\.labelinsight\.com\/product\//i.test(href) ||
            /smartlabel\.org\/product\//i.test(href)
          )
      );
      if (!hrefs.length && isSmartLabelOrg) break;
      ...
      if (isSmartLabelOrg) {
        ...
        const nextButton =
          page.getByRole("button", { name: /next page/i }).first().or(
            page.locator("a").filter({ hasText: "Next Page" }).first()
          ).or(
            page.locator("div.page-link", { hasText: "Next Page" }).first()
          );
        if (!(await nextButton.count())) break;
        const ariaDisabled = await nextButton.getAttribute("aria-disabled", { timeout: 3000 }).catch(() => null);
        const className = await nextButton.getAttribute("class");
        const isDisabled =
          ariaDisabled === "true" ||
          (className ? /\bdisabled\b/i.test(className) : false) ||
        ...
        await page
          .waitForFunction(
            (prev) => {
              const first = document.querySelector("a[href*='smartlabel.']") as HTMLAnchorElement | null;
...
async function fetchProduct(
  ...
  const labelInsightMatch = url.match(/smartlabel\.labelinsight\.com\/product\/(\d+)/i);
  const isLabelInsight = !!labelInsightMatch;
  const labelInsightId = labelInsightMatch?.[1] ?? null;
  const labelInsightBase = labelInsightId
    ? `https://smartlabel.labelinsight.com/product/${labelInsightId}`
    : null;
  if (!isLabelInsight) {
    if (DEBUG_SMART_LABEL) {
      console.log(`[DEBUG] skipped ${url}: not a labelinsight product url`);
    }
    return null;
  }
  ...
  let labelInsightHtml = html;
  let labelInsightJson = isLabelInsight ? extractLabelInsightProductJson(labelInsightHtml) : null;
  if (isLabelInsight && !labelInsightJson) {
    const ingredientsUrl = labelInsightBase ? `${labelInsightBase}/ingredients` : url;
    const ingredientsHtml = await fetchWithFallback(ingredientsUrl).catch(() => "");
    if (ingredientsHtml) {
      labelInsightHtml = ingredientsHtml;
      labelInsightJson = extractLabelInsightProductJson(labelInsightHtml);
      if (DEBUG_SMART_LABEL) {
        await fs.writeFile("/tmp/smart-label-ingredients-http.html", ingredientsHtml).catch(() => null);
      }
    }
    if (!labelInsightJson) {
      if (DEBUG_SMART_LABEL) {
        console.log(`[DEBUG] axios fallback -> playwright render for ingredients: ${ingredientsUrl}`);
      }
      ...
      if (renderedIngredients) {
        labelInsightHtml = renderedIngredients;
        labelInsightJson = extractLabelInsightProductJson(labelInsightHtml);
      }
    }
  }
  ...
  const $label = isLabelInsight ? cheerio.load(labelInsightHtml) : $;
  const headerName = extractHeaderName($label);
  const headerBrand = extractHeaderBrand($label);
  const rawName =
    headerName ||
    (labelInsightJson?.title as string | undefined) ||
    extractName($label);
  ...
  const brand =
    ((useHeaderBrand ? headerBrand : derived.brand) ||
      (labelInsightJson?.brand as string | undefined) ||
      extractBrand($) ||
      "Unknown"
    ).trim();
  ...
  const labelInsightRawIngredients = labelInsightJson?.rawIngredients
    ? normalizeWhitespace(String(labelInsightJson.rawIngredients))
    : "";
  const labelInsightIngredientList = Array.isArray(labelInsightJson?.ingredientSection?.ingredients)
    ? labelInsightJson.ingredientSection.ingredients
    : [];
  if (labelInsightRawIngredients) {
    ingredientsText = labelInsightRawIngredients;
    ingredientsSource = "rawIngredients";
  } else if (isLabelInsight) {
    ingredientsText = extractLabelInsightIngredientsFromJson(labelInsightJson);
    if (ingredientsText) ingredientsSource = "labelinsight-json";
  }
  if (!ingredientsText && isLabelInsight) {
    ingredientsText = extractLabelInsightIngredients($);
    if (ingredientsText) ingredientsSource = "labelinsight-dom";
  }
  if (
    isLabelInsight &&
    !ingredientsText &&
    !labelInsightRawIngredients &&
    labelInsightIngredientList.length === 0
  ) {
    if (DEBUG_SMART_LABEL) {
      console.log(`[DEBUG] skipped ${url}: no ingredients in labelinsight payload`);
    }
    return null;
  }
  ...
  if (isLabelInsight) {
    const marketingImage = labelInsightJson?.marketingImage as
      | { high?: string; medium?: string; small?: string }
      | undefined;
    const imagesArray = Array.isArray(labelInsightJson?.images) ? labelInsightJson.images : [];
    const arrayImage =
      imagesArray?.[0]?.highResolutionUrl ||
      imagesArray?.[0]?.mediumResolutionUrl ||
      imagesArray?.[0]?.lowResolutionUrl;
    if (marketingImage?.high) {
      imageUrl = String(marketingImage.high);
    } else if (arrayImage) {
      imageUrl = String(arrayImage);
    } else if (marketingImage?.medium) {
      imageUrl = String(marketingImage.medium);
    }
  }
  if (!imageUrl && isLabelInsight && labelInsightBase) {
    const imageHref = $label("a[href^='/product/'][href*='/image']").first().attr("href");
    if (imageHref) {
      imageUrl = imageHref.startsWith("http") ? imageHref : `${baseOrigin}${imageHref}`;
    } else {
      imageUrl = `${labelInsightBase}/image`;
    }
  }
  if (!imageUrl) {
    imageUrl = extractImage($label, baseOrigin);
  }
  if (!imageUrl && labelInsightJson?.marketingImage?.high) {
    imageUrl = String(labelInsightJson.marketingImage.high);
  }

  const upcFromText =
    (labelInsightJson?.upc as string | undefined) ||
    extractUpc(html) ||
    extractUpc($("body").text());
  ...
  if (!nutrition && labelInsightJson) {
    const parsed = parseLabelInsightNutritionJson(labelInsightJson);
  ...
  if (!allergenStatement) {
    const allergensUrl =
      isLabelInsight && labelInsightBase ? `${labelInsightBase}/allergens` : (url.includes("#") ? url : `${url}#allergens`);
    let allergensHtml = await fetchWithFallback(allergensUrl).catch(() => "");
    if (!allergensHtml) {
      if (DEBUG_SMART_LABEL) {
        console.log(`[DEBUG] axios fallback -> playwright render for allergens: ${allergensUrl}`);
      }
      allergensHtml = await fetchRenderedHtml(browser, allergensUrl);
    }
    const $a = cheerio.load(allergensHtml);
