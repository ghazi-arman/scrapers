...
const SCRAPER_NAME = process.env.JOB_NAME || "smart-label-conagra";
...
function parseSmartLabelNutrition($: cheerio.CheerioAPI): { nutrition: ScraperNutritionData | null; servingSizeText: string | null } {
  ...
  const perServingIndex = (() => {
    if (servingPanel.length) return null;
    const labelEl = scope
      .find("*")
      .filter((_, el) => /per\s*serving/i.test($(el).text()))
      .first();
    ...
    const siblings = parent
      ...
      .filter(Boolean);
    const labelText = normalizeWhitespace(labelEl.text());
    const idx = siblings.findIndex((t) => t === labelText);
  ...
  let caloriesText = caloriesFromTable || caloriesFromTop || "";
  if (caloriesText) {
  ...
  scope.find(".nfp__row").each((_, row) => {
    ...
    let valueText = "";
    if (perServingIndex != null && valueCandidates[perServingIndex]) {
      valueText = valueCandidates[perServingIndex];
    } else {
      valueText =
        valueCandidates.find((v) => !/%/.test(v)) ||
        valueCandidates.find((v) => /<|>|≈|~|\d/.test(v)) ||
        "";
    }
    if (!valueText) {
      ...
      if (!isSmartLabelOrg) {
        const hrefs = await page.$$eval("a[href]", (links) =>
          links
            ...
            .filter((href) => /smartlabel\.conagra\.com/i.test(href))
        );
...
async function fetchProduct(
  ...
  try {
    ...
    if (status === 403) {
      console.warn(`[SKIP] 403 Forbidden: ${url}`);
      return null;
    }
    if (DEBUG_SMART_LABEL) {
      console.error(`[DEBUG] base fetch failed for ${url}, trying rendered fallback:`, err);
    }
    html = await fetchRenderedHtml(browser, url);
    if (!html) {
      console.warn(`[SKIP] failed to fetch product HTML: ${url}`);
      return null;
    }
  }
  if (DEBUG_SMART_LABEL) {
...
async function main(): Promise<void> {
  ...
  if (effectiveSearchUrl && !ranConfigSearch && urls.length < desiredCount) {
    try {
      const remaining = Number.isFinite(desiredCount) ? Math.max(desiredCount - urls.length, 0) : undefined;
